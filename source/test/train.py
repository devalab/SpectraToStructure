...
def loss_pi(predicted,target):
    # loss = torch.sum(-target*(((1e-8+predicted)/(1e-8+target)).log()),1)
...
def train(model, value_model,episode_actor,wandb):
    ...
    model.to(device1)
    # value_model.to(device2)

    while(ray.get(episode_actor.get_size.remote()) < PRETRAINING_SIZE):
        # print(ray.get(episode_actor.get_size.remote()))
        True
    print("--------Training Started--------")
    while(1):
        # print(ray.get(episode_actor.get_lock.remote()),ray.get(episode_actor.get_queue.remote()))
        if (ray.get(episode_actor.get_size.remote()) >= PRETRAINING_SIZE ): 
            ...
            # value_loader = DataLoader(train_dataset, collate_fn=collate, batch_size=BATCH_SIZE, shuffle=True)

            # train_value(1, value_model, value_optimizer, value_loader,wandb,value_file)

            # torch.save(model.state_dict(), "./saved_models/prior.state")
            # torch.save(value_model.state_dict(), "./saved_models/value_model.state")

        if (not ray.get(episode_actor.get_lock.remote())) and (not ray.get(episode_actor.get_queue.remote())):
            time.sleep(0.2)
            model.to(torch.device("cpu"))
            # value_model.to(torch.device("cpu"))
            print("------------model sending--------------")
...
def train_prior(epochs,model,optimizer,database,wandb,file):
    ...
    print("--------Training Started--------")
    # print("before train loop wrgiht: ", model.state_dict()['final.bias'])
    for i in range(epochs):
        # print("Training Epoch : {}  Datasize: {}".format(i,MyDatabase.size))
        for samples in database:
        # for sb,Vb,pib,fpib in MyDatabase:
            optimizer.zero_grad()
            data,pib,vb = samples
            output = model(data)
    ...
    sys.stdout.flush()
    file.write("Train Prior Loss: "+ str(np.mean(loss_arr)) +"\n")
    # print("after train loop wrgiht: ", model.state_dict()['final.bias'])
